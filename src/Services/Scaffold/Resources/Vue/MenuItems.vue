<template>
    ...
    <div v-for="(item, index) of items"
         ...
         class="relative tracking-tight font-medium whitespace-nowrap flex items-center h-12 px-5 duration-150 cursor-pointer hover:text-primary transition-color"
         :class="[
            selected === item.label ? 'text-primary' : 'text-gray-700',
            collapsed ? 'w-16': 'w-56'
         ]">

        <RouterLink v-if="item.route" :to="item.route" class="flex items-center justify-center">
        ...
        <div v-else class="flex items-center justify-center">
            ...
            <div v-if="selected === item" class="absolute left-0 w-1 h-12 rounded-r bg-primary"/>
